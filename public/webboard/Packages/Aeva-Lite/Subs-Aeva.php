...
{
	global $context, $modSettings, $sourcedir, $sites, $upto, $user_info;
	...
	// Attempt to Load - Enabled Sites, if not already loaded
	if (empty($sites))
	...
	// By the way, if you're installing Aeva Media -- uninstall Aeva Lite first!
	if (isset($_REQUEST, $_REQUEST['action'], $_REQUEST['sa']) && $_REQUEST['action'] == 'mgallery' && $_REQUEST['sa'] == 'post')
...
function aeva_build_object($input)
{
	...
	if ($context['aeva']['remaining'] == 0)
		return preg_replace(array('~http://~i', '@#[\w/\.~-]*@'), array('noae://', ''), $input[0], 1) . ' ' . $txt['aeva_too_many_embeds'];
	...
	elseif ($arr['plugin'] == 'realmedia')
	{
		$plugin = array(
			...
			'src' => 'src',
			'extra' => (($context['browser']['is_ie'] && !$context['browser']['is_mac_ie'] ? '<object id="rvocx" classid="CLSID:CFCDAA03-8BE4-11CF-B84B-0020AFBBCCFA" width="$8px" height="30px"><param name="src" value="$1"><param name="autostart" value="false"><param name="controls" value="ControlPanel"><param name="console" value="video">' : '' ) .
				'<embed src="$1" width="$8px" height="30" controls="ControlPanel" type="audio/x-pn-realaudio-plugin" console="video" autostart="false"></embed>' . ($context['browser']['is_ie'] && !$context['browser']['is_mac_ie'] ? '</object>' : '')),
		);
	}
	...
	foreach ($opt as $ion)
	{
		...
		if ($ion == 'noexternalembed')
			return preg_replace(array('~http://~i', '@#noext[\w/\.~-]*@'), array('noae://', ''), $input[0]) . ' ' . $txt['aeva_noexternalembedding'];
	...
	$show_raw_link = (isset($title) && (empty($modSettings['aeva_titles']) || $modSettings['aeva_titles'] < 2))
				  || (!empty($modSettings['aeva_includeurl']) && !empty($arr['show-link']) && empty($context['mg_calling']));
...
<tr><td style="width: ' . $swp . '; height: ' . $shp . '"' . ($show_something_below ? ' colspan="2"' : '') . ' id="saeva' . $swfobjects . '">';
	...
	') . '</div>
	<script type="text/javascript"><!-- // --><![CDATA[' . ($swfobjects === 1 ? '
		var oldLoad = window.onload;
...
<tr><td class="aeva_t">';
		...
		// Include the original link? Don't do it if SMG is calling for integration, because it would create a double embed.
		if ($show_raw_link)
...
function aeva_parse_bbc2(&$message, &$smileys, &$cache_id)
{
	...
	if (strlen($message) > 10 & empty($context['uninstalling']))
	{
		...
		if (!empty($context['user']['is_admin']) && empty($context['aeva_disable']) && !empty($modSettings['aeva_enable']) && !empty($modSettings['aeva_debug']) && strpos($cache_id, 'sig') === false)
		{
			// We need our language strings
			if (!isset($txt['aeva_debug_took']) && loadLanguage('Aeva') == false)
				loadLanguage('Aeva', 'english');
			// Append the timer to each post
			$message .= '<br /><div class="smalltext">' . $txt['aeva_debug_took'] . ' ' . round($aeva_timer + array_sum(explode(' ', microtime())), 4) . $txt['aeva_debug_seconds'] . '</div>';
		}
	}
...
// The next functions are fittingly fetched and modified from my own code in Aeva Media, and SMF2 for aeva_fetch()
...
// PHP<5 has no str_ireplace, so use our own optimized version
// Modified from http://uk.php.net/manual/en/function.str-ireplace.php#72719
