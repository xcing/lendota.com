...
(function($) {
  var jqueryBracket = function(opts)
  {
    ...
    var Match = function(round, data, idx, results, renderCb)
    {
      ...
      function winner() {
        if (isNumber(data[0].score) && isNumber(data[1].score) && isMatchFinished()) {
      ...
      function isMatchFinished() {
          ...
          if (results && results[3] == 'final') {
              isFinalRound = true;
          }
          var gameAmount = isFinalRound ? opts.finalGameAmount : opts.gameAmount;
          var gameToWin = (Math.floor(gameAmount / 2) + 1);
          if (data[0].score >= gameToWin || data[1].score >= gameToWin) {
              return true;
          }
          return false;
      }
      ...
      function teamElement(round, team, isReady) {
        ...
        if (team.link) {
            var matchLink = $('<a href="'+team.link+'"></a>');
            matchLink.append(tEl);
            tEl = matchLink;
        }
        return tEl;
      }
      ...
      if (results && results.length > 2) {
          data[0].link = results[2];
          data[1].link = results[2];
      }
...
//      }
      ...
      if (isSingleElimination) {
        ...
        if (teams.length > 1) {
          ...
          consol.setAlignCb(function(tC) {
            ...
            if (!opts.showThirdPlace) {
                hideThirdPlace(consol);
            }
    ...
    function hideThirdPlace(consol) {
      consol.el.css('display', 'none');
    }
